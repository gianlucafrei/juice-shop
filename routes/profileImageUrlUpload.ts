...
module.exports = function profileImageUrlUpload () {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if(invalid_url(req.body.imageUrl)){
      next(new Error('SSRF protection, address must not start with 127.0.0.1 or localhost'));
      return;
    }
    ...
    if (loggedInUser) {
      ...
      const options = {
        url: url,
        headers: {
          'User-Agent': 'JuiceShop' // This is needed to verify the ssrf challenge
        }
      };
